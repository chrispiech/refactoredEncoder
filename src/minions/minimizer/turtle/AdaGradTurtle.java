...
public class AdaGradTurtle {
	...
	public static Encoder train(ModelFormat f, String expName, int maxHours,
			String name) {
		AdaGradTurtle min = new AdaGradTurtle();
		min.format = f;
		min.name = name;
	...
	private double[] SGD() {
		...
		while(timeHasNotExpired()){
			...
			for(int t = 0; t < batches.size(); t++) {
				List<String> miniBatch = batches.get(t);

				Map<String, TurtleMatrix> matrixValues = getMatrixValues(miniBatch);

				TurtleModel model = (TurtleModel) getModel(model_x);
				
				double sumMiniError = 0;
				double sumMiniAccuracy = 0;
				int miniTested = 0;

				// update a single mini-batch
				double [] model_grad = new double[model_x.length];
				for(String matrixKey : miniBatch) {
					...
					// update each of the program matricies
					...
					for(TestTriplet test : matrixTests) {
						...
						for(int i = 0; i < model_grad.length; i++) {
							model_grad[i] += model_dxdp[i];
						}
						
						
						sumMiniError += getError(model, test, matrix);
						sumMiniAccuracy += getAccuracy(model, test, matrix);
						miniTested++;
					}

					// AdaGrad update (and save to disk) for the program matrix
					adaGradMatrix(matrixValues, matrixKey, matrixGrad);
				}

				// AdaGrad update for the model vector
				adaGradModel(eta, model_x, model_gs, model_grad);
			...
			epochOutput(sumError, sumAccuracy, numTested);
			epoch++;
		}
	...
	private double getAccuracy(TurtleModel model, TestTriplet test,
		...
		State guess = model.getGuess(test, matrix);
		return ModelTester.calcAccuracyPartialCredit(format, test.getPostcondition(), guess);
	}
	...
	private void epochOutput(double sumError, double sumAccuracy, int numTested) {
	...
	private void loadTrainSet() {
		...
		try {
			...
			while (codeIn.hasNextLine()) {
				...
				if(line.split(",")[0].length() > 1) continue;
	...
	private double getTestAccuracy(Encoder model) {
		return ModelTester.calcAccuracyPartialCredit(model, null);
	}
