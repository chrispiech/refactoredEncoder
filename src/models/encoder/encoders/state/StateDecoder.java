...
public class StateDecoder implements StateDecoderI{
	...
	public double getLogLoss(State state, SimpleMatrix stateVector, int count) {
		double logLoss = 0;
		for(String key : getStateKeys()) {
			ValueDecoder outDecoder = getOutputDecoder(key);
			logLoss += count * outDecoder.logLoss(state, stateVector);
		}
		return logLoss;
	}
